...
from .block import Block, Children, CollectionViewBlock, PageBlock
...
class NotionDate(object):
    ...
    def __init__(
        self, start, end=None, timezone=None, reminder: Optional[Reminder] = None
    ):
        self.start = start
        self.end = end
...
class Collection(Record):
    ...
    def check_schema_select_options(self, prop, values):
        ...
        """
        schema_update = False
        current_options = [p["value"].lower() for p in prop.get("options", [])]
...
class CollectionView(Record):
    ...
    def update_view_sort(self, sort_list):
        ...
        with self._update_query2() as query2_content:
            ...
            for sort in sort_list:
                assert (
                    type(sort) is list
                ), 'sort_list has invalid format. Expected [["property", "direction"],...] '
    ...
    def _submit_view_transaction(self, args, path=None):
        ...
        self._client.submit_transaction(
            ...
            generate_update_last_edited=False,
            updated_blocks=[self.parent.id],
        )
    ...
    def generate_aggregation_query(self, aggregations):
        ...
        for aggregation in aggregations:
            assert type(aggregation) is list, "Aggregation {} must be list".format(
                str(aggregation)
            )
            property_id = self.collection.get_schema_property(aggregation[0])["id"]
            aggregation_list.append(
                {"property": property_id, "aggregator": aggregation[1]}
            )
    ...
    def generate_view_filter_query(self, filters):
        ...
        for filter_dict in filters:
            ...
            for group_operator, filter_group in filter_dict.items():
                ...
                for filter in filter_group:
                    assert type(filter) is list, "Filter {} must be list".format(
                        str(filter)
                    )
                    assert (
                        len(filter) == 4
                    ), "Filter {} must contain [slugfied property, operator, type, value]".format(
                        str(filter)
                    )
                    ...
                    filters_list.append(
                        {
                            ...
                            "filter": {
                                "operator": filter[1],
                                "value": {"type": filter[2], "value": filter[3]},
                            },
                        }
                    )
        ...
        main_operator = (
            main_filter[0] if type(main_filter) is list else main_filter["operator"]
        )
    ...
    def _update_query2(self):
        try:
            self._client.refresh_records(
                collection_view=self.id, block=self.collection.id
            )
...
class BoardView(CollectionView):
    ...
    def update_group_by(self, group_property):
        ...
        with self._update_query2() as query2_content:
            query2_content["group_by"] = self.collection.get_schema_property(
                group_property
            )["id"]
    ...
    def format_properties(
        ...
        column_values_hidden,
        board_cover=None,
        board_cover_size=None,
        board_cover_aspect=None,
    ):
        """
        ...
        with self._update_format() as format_content:
            find_idx = lambda prop: [
                True if x["property"] == prop else False
                for x in format_content["board_properties"]
            ].index(True)
            ...
            for property in self.collection.get_schema_properties():
                ...
                try:
                    ...
                    format_content["board_properties"].append(
                        {"property": property_id, "visible": True, "width": 200}
                    )
                    idx = len(format_content["board_properties"]) - 1

                if property["slug"] in properties_visibility:
                    format_content["board_properties"][idx][
                        "visible"
                    ] = properties_visibility[property["slug"]]
                        ...
                        board_groups.append(
                            {
                                ...
                                "hidden": column_values_hidden["default"],
                            }
                        )
            ...
            if board_cover is not None:
                ...
                if is_property is not None:
                    format_content["board_cover"] = {
                        "type": "property",
                        "property": is_property["id"],
                    }
                else:
                    format_content["board_cover"] = {"type": board_cover}

            format_content["board_cover_size"] = (
                board_cover_size
                if board_cover_size is not None
                else format_content["board_cover_size"]
            )
            format_content["board_cover_aspect"] = (
                board_cover_aspect
                if board_cover_aspect is not None
                else format_content["board_cover_aspect"]
            )
            format_content["board_groups2"] = (
                board_groups
                if board_groups is not None
                else format_content["board_groups2"]
            )
...
class TableView(CollectionView):
    ...
    def format_properties(self, properties_visibility, properties_width=None):
        ...
        with self._update_format() as format_content:
            find_idx = lambda prop: [
                True if x["property"] == prop else False
                for x in format_content["table_properties"]
            ].index(True)
            ...
            for property in self.collection.get_schema_properties():
                ...
                try:
                    ...
                    format_content["table_properties"].append(
                        {"property": property_id, "visible": True, "width": 200}
                    )
                    idx = len(format_content["table_properties"]) - 1

                if property["slug"] in properties_visibility:
                    format_content["table_properties"][idx][
                        "visible"
                    ] = properties_visibility[property["slug"]]

                if property["slug"] in properties_width:
                    format_content["table_properties"][idx]["width"] = properties_width[
                        property["slug"]
                    ]
...
class ListView(CollectionView):
    ...
    def format_properties(self, properties_visibility, on_first_load_show=None):
        ...
        with self._update_format() as format_content:
            find_idx = lambda prop: [
                True if x["property"] == prop else False
                for x in format_content["list_properties"]
            ].index(True)
            ...
            if on_first_load_show is not None:
                format_content["inline_collection_first_load_limit"] = {
                    "type": "load_limit",
            ...
            for property in self.collection.get_schema_properties():
                ...
                try:
                    ...
                    format_content["list_properties"].append(
                        {"property": property_id, "visible": True}
                    )
                    idx = len(format_content["list_properties"]) - 1

                if property["slug"] in properties_visibility:
                    format_content["list_properties"][idx][
                        "visible"
                    ] = properties_visibility[property["slug"]]
...
class CalendarView(CollectionView):
    ...
    def update_calendar_by(self, date_property):
        ...
        with self._update_query2() as query2_content:
            query2_content["calendar_by"] = self.collection.get_schema_property(
                date_property
            )["id"]
    ...
    def format_properties(self, properties_visibility):
        ...
        with self._update_format() as format_content:
            find_idx = lambda prop: [
                True if x["property"] == prop else False
                for x in format_content["calendar_properties"]
            ].index(True)
            ...
            for property in self.collection.get_schema_properties():
                ...
                try:
                    ...
                    format_content["calendar_properties"].append(
                        {"property": property_id, "visible": True}
                    )
                    idx = len(format_content["calendar_properties"]) - 1

                if property["slug"] in properties_visibility:
                    format_content["calendar_properties"][idx][
                        "visible"
                    ] = properties_visibility[property["slug"]]
...
class GalleryView(CollectionView):
    ...
    def format_properties(
        self,
        properties_visibility,
        on_first_load_show=None,
        gallery_cover=None,
        gallery_cover_size=None,
        gallery_cover_aspect=None,
    ):
        """
        Format TableView properties.
        properties_visibility: dict - {"property" : visible}
        ...
        with self._update_format() as format_content:
            find_idx = lambda prop: [
                True if x["property"] == prop else False
                for x in format_content["gallery_properties"]
            ].index(True)
            ...
            for property in self.collection.get_schema_properties():
                ...
                try:
                    ...
                    format_content["gallery_properties"].append(
                        {"property": property_id, "visible": True}
                    )
                    idx = len(format_content["gallery_properties"]) - 1

                if property["slug"] in properties_visibility:
                    format_content["gallery_properties"][idx][
                        "visible"
                    ] = properties_visibility[property["slug"]]
            ...
            format_content["gallery_cover_size"] = (
                gallery_cover_size
                if gallery_cover_size is not None
                else format_content["gallery_cover_size"]
            )
            format_content["gallery_cover_aspect"] = (
                gallery_cover_aspect
                if gallery_cover_aspect is not None
                else format_content["gallery_cover_aspect"]
            )
...
class TimelineView(CollectionView):
    ...
    def update_timeline_by(self, date_property_start, date_property_end=None):
        ...
        with self._update_query2() as query2_content:
            query2_content["timeline_by"] = self.collection.get_schema_property(
                date_property_start
            )["id"]

            if date_property_end is not None:
                query2_content["timeline_by_end"] = self.collection.get_schema_property(
                    date_property_end
                )["id"]
    ...
    def format_properties(
        ...
        timeline_table_properties_visibility = {
            slugify(key): v for key, v in timeline_table_properties_visibility.items()
        }
        timeline_properties_visibility = {
            slugify(key): v for key, v in timeline_properties_visibility.items()
        }
        timeline_table_properties_width = {
            slugify(key): v for key, v in timeline_table_properties_width.items()
        }

        with self._update_format() as format_content:
            find_idx = lambda prop: [
                True if x["property"] == prop else False
                for x in format_content["timeline_properties"]
            ].index(True)

            find_idx_table = lambda prop: [
                True if x["property"] == prop else False
                for x in format_content["timeline_table_properties"]
            ].index(True)
            ...
            if timeline_preference is not None:
                format_content["timeline_preference"] = {
                    "zoomLevel": timeline_preference[0],
                    "centerTimestamp": timeline_preference[1].timestamp(),
                }
            ...
            for property in self.collection.get_schema_properties():
                ...
                try:
                    ...
                    format_content["timeline_properties"].append(
                        {"property": property_id, "visible": True}
                    )
                    idx = len(format_content["timeline_properties"]) - 1

                if property["slug"] in timeline_properties_visibility:
                    format_content["timeline_properties"][idx][
                        "visible"
                    ] = timeline_properties_visibility[property["slug"]]

                try:
                    idx = find_idx_table(property_id)
                except:
                    format_content["timeline_table_properties"].append(
                        {"property": property_id, "visible": True, "width": 200}
                    )
                    idx = len(format_content["timeline_table_properties"]) - 1

                if property["slug"] in timeline_table_properties_visibility:
                    format_content["timeline_table_properties"][idx][
                        "visible"
                    ] = timeline_table_properties_visibility[property["slug"]]

                if property["slug"] in timeline_table_properties_width:
                    format_content["timeline_table_properties"][idx][
                        "width"
                    ] = timeline_table_properties_width[property["slug"]]
        ...
        if self.aggregate is not None:
            for entry in self.aggregate:
                ...
                kwargs["aggregate"].append(
                    {"aggregation": entry, "type": "aggregation", "key": key}
                )
    ...
    def set_property(self, identifier, val, reminder: Optional[Reminder] = None):
        ...
        if reminder and prop["type"] == "date":
            self.get_property(identifier).reminder = dict(
                **reminder, time=reminder.get("time", None)
            )
